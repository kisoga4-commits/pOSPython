...
function customerScanUrl(tableId) { return `${window.location.origin}/scan/customer/${tableId}`; }
function buildQrImageUrl(text) { return `${qrApiBase}?size=300x300&margin=8&data=${encodeURIComponent(text)}`; }

async function api(path, options = {}) {
  const res = await fetch(path, { headers: { 'Content-Type': 'application/json' }, ...options });
  const text = await res.text();
  let data = {};
  try { data = text ? JSON.parse(text) : {}; } catch { data = { error: 'invalid_json', raw: text }; }
  if (!res.ok) return { error: data.error || `request_failed_${res.status}`, status: res.status, ...data };
  return data;
}
...
function applyTheme() {
  ...
  qs('store-logo').textContent = s.logoName || 'LOGO';
}
...
function formatDateTime(value) { return value ? new Date(value).toLocaleString('th-TH', { hour12: false }) : '-'; }
...
function renderTables() {
  ...
  db.tables.forEach((table) => {
    ...
    card.type = 'button';
    card.className = `table-card ${meta.tone} ${table.status} ${selectedTableId === table.id ? 'is-selected' : ''}`;
    card.innerHTML = `
      <div class="table-head-row"><p class="table-no">${unit} ${table.id}</p><span class="status-chip ${meta.tone}">${meta.label}</span></div>
...
function waitingMinutes(openedAt) {
  if (!openedAt) return '-';
  return `${Math.max(1, Math.round((Date.now() - new Date(openedAt).getTime()) / 60000))} นาที`;
}

function buildPromptPayPayload(promptPayId, amount) {
  const pp = String(promptPayId || '').replace(/[^0-9A-Za-z]/g, '');
  return pp ? `PP|${pp}|${Number(amount || 0).toFixed(2)}` : '';
}
...
async function openBill(target, targetId) {
  ...
  (activeBill.items || []).forEach((item) => {
    ...
    row.innerHTML = `<strong>${item.name}</strong><div>${money(item.price)} บาท ${item.addon ? `· add-on: ${item.addon}` : ''}</div>`;
    list.appendChild(row);
  });
  qs('bill-total').textContent = money(activeBill.total);
  const qrPreview = qs('bill-qr-preview');
  if (db.settings?.dynamicPromptPay) {
    ...
    qrPreview.classList.remove('hidden');
    qrPreview.innerHTML = `<div class="list-card"><strong>Static QR</strong><img src="${db.settings.qrImage}" class="qr-preview" alt="Static QR" /></div>`;
  } else {
...
function renderCashier() {
  ...
  if (!queues.length) {
    ...
    qs('cashier-empty').classList.remove('hidden');
    qs('cashier-bill').classList.add('hidden');
    activeCashierTableId = null;
    return;
  }
  queues.forEach((table) => {
    ...
    row.type = 'button';
    row.className = `list-card table-card ${activeCashierTableId === table.id ? 'is-selected' : ''}`;
    row.innerHTML = `<strong>${unitLabel()} ${table.id}</strong><div>สถานะ: ${(statusMap[table.status] || statusMap.available).label}</div><div>ยอดรวม ${money(total)} บาท · รอ ${waitingMinutes(firstOrder?.created_at)}</div>`;
    row.addEventListener('click', () => openBill('table', table.id));
...
function createInsights() {
  ...
  const previous = filteredSales.filter((s) => { const d = new Date(s.paid_at); return d >= prev7 && d < start7; });
  const itemCount = {};
  filteredSales.forEach((sale) => (sale.items || []).forEach((it) => { itemCount[it.name] = (itemCount[it.name] || 0) + 1; }));
  return { currentTotal: sum(current), previousTotal: sum(previous), diff: sum(current) - sum(previous), topItems: Object.entries(itemCount).sort((a, b) => b[1] - a[1]).slice(0, 5) };
}
...
function renderSales() {
  ...
  qs('cash-total').textContent = money(sum(filteredSales.filter((s) => s.payment_method === 'cash')));
  qs('qr-total').textContent = money(sum(filteredSales.filter((s) => s.payment_method === 'qr')));
  qs('grand-total').textContent = money(sum(filteredSales));

  const history = qs('sales-history'); history.innerHTML = '';
  ...
  filteredSales.slice().reverse().forEach((s) => {
    ...
    row.textContent = `${new Date(s.paid_at).toLocaleString('th-TH')} · ${s.payment_method} · ${money(s.total)} บาท`; history.appendChild(row);
  });

  const hit = {};
  filteredSales.forEach((sale) => (sale.items || []).forEach((it) => { hit[it.name] = (hit[it.name] || 0) + 1; }));
  const hot = qs('hot-menu'); hot.innerHTML = '';
  ...
  sorted.forEach(([name, qty]) => { const row = document.createElement('div'); row.className = 'list-card'; row.textContent = `${name} · ${qty} รายการ`; hot.appendChild(row); });
  ...
  qs('insight-body').innerHTML = `<div class="list-card">ช่วงล่าสุด: ${money(insight.currentTotal)} บาท</div><div class="list-card">ช่วงก่อนหน้า: ${money(insight.previousTotal)} บาท</div><div class="list-card">ส่วนต่าง: ${insight.diff >= 0 ? '+' : '-'}${money(Math.abs(insight.diff))} บาท</div><div class="list-card">Top items: ${insight.topItems.map(([name, qty]) => `${name} (${qty})`).join(', ') || '-'}</div>`;
}

function renderMenu() {
  const list = qs('menu-list'); list.innerHTML = '';
  if (!db.menu.length) { list.innerHTML = '<div class="empty">ยังไม่มีเมนู กด + เพิ่ม เพื่อเริ่มต้น</div>'; return; }
  db.menu.forEach((item, idx) => {
    ...
    row.querySelector('[data-a="e"]').addEventListener('click', () => {
      menuEditIndex = idx; qs('menu-name').value = item.name; qs('menu-price').value = item.price; qs('menu-image').value = item.image || ''; qs('menu-addons').value = (item.addons || []).join(', ');
    });
...
function renderTableQRCodes() {
  const wrap = qs('table-qr-list'); if (!wrap) return; wrap.innerHTML = '';
  (db.tables || []).forEach((table) => {
    const tableUrl = customerScanUrl(table.id); const qrImageUrl = buildQrImageUrl(tableUrl);
    const card = document.createElement('div'); card.className = 'qr-table-card';
    card.innerHTML = `<strong>${unitLabel()} ${table.id}</strong><img src="${qrImageUrl}" alt="QR ${unitLabel()} ${table.id}" loading="lazy" /><a class="btn-soft" href="${tableUrl}" target="_blank" rel="noopener">เปิดลิงก์ลูกค้า</a><a class="btn-soft" href="${qrImageUrl}" download="qr-${unitLabel()}-${table.id}.png">ดาวน์โหลด QR</a>`;
...
function selectMenuItem(item) {
  ...
  const addon = qs('order-addon'); addon.innerHTML = '<option value="">+ add-on</option>';
  (item.addons || []).forEach((add) => { const option = document.createElement('option'); option.value = add; option.textContent = add; addon.appendChild(option); });
  document.querySelectorAll('#order-menu-grid .menu-choice').forEach((btn) => btn.classList.toggle('is-active', Number(btn.dataset.menuId) === Number(item.id)));
}

function renderOrderMenuChoices() {
  const grid = qs('order-menu-grid'); grid.innerHTML = '';
  db.menu.forEach((item) => {
    const btn = document.createElement('button'); btn.type = 'button'; btn.className = 'menu-choice'; btn.dataset.menuId = item.id;
    btn.innerHTML = `<strong>${item.name}</strong><small>฿${money(item.price)}</small>`;
    btn.addEventListener('click', () => selectMenuItem(item)); grid.appendChild(btn);
  });
}

function renderOrderCart() {
  const list = qs('order-cart-list'); list.innerHTML = '';
  if (!orderCart.length) { list.innerHTML = '<div class="empty order-empty">ยังไม่มีรายการในตะกร้า</div>'; qs('order-cart-total').textContent = 'Subtotal ฿0.00'; return; }
  orderCart.forEach((item, idx) => {
    const row = document.createElement('div'); row.className = 'order-line-item';
    row.innerHTML = `<div class="order-line-main"><div><div class="order-line-name">${item.name}</div><div class="order-line-meta">${item.addon ? `+ ${item.addon}` : 'ไม่เพิ่ม add-on'} ${item.note ? `• ${item.note}` : ''}</div></div><div class="order-line-price">฿${money(Number(item.price) * Number(item.qty))}</div></div><div class="order-line-actions"><button class="order-mini-btn" data-act="minus">−</button><span class="order-line-meta">${item.qty} จาน</span><button class="order-mini-btn" data-act="plus">＋</button><button class="order-mini-btn danger" data-act="remove">ลบ</button></div>`;
    row.querySelector('[data-act="minus"]').addEventListener('click', () => { item.qty = Math.max(1, Number(item.qty) - 1); renderOrderCart(); });
    row.querySelector('[data-act="plus"]').addEventListener('click', () => { item.qty = Number(item.qty) + 1; renderOrderCart(); });
    row.querySelector('[data-act="remove"]').addEventListener('click', () => { orderCart.splice(idx, 1); renderOrderCart(); });
    list.appendChild(row);
  });
  qs('order-cart-total').textContent = `Subtotal ฿${money(orderCart.reduce((sum, item) => sum + (Number(item.price) * Number(item.qty)), 0))}`;
}

function renderExistingOrders(tableId) {
  const list = qs('order-existing-list'); const { items, total, latestOrder } = getTableSummary(tableId); list.innerHTML = '';
  if (!items.length) list.innerHTML = '<div class="empty order-empty">ยังไม่มีรายการที่สั่งแล้ว</div>';
  else items.forEach((item) => {
    const row = document.createElement('div'); row.className = 'order-line-item';
    row.innerHTML = `<div class="order-line-main"><div class="order-line-name">${item.name}</div><div class="order-line-price">฿${money((item.qty || 1) * Number(item.price || 0))}</div></div><div class="order-line-meta">${item.qty || 1} จาน ${item.addon ? `• + ${item.addon}` : ''} ${item.note ? `• ${item.note}` : ''}</div>`;
    list.appendChild(row);
  });
...
async function submitOrderFromPanel() {
  ...
  orderCart.forEach((item) => {
    for (let i = 0; i < Number(item.qty || 1); i += 1) payloadItems.push({ name: item.name, price: Number(item.price), addon: item.addon, note: item.note, qty: 1 });
  });
  const res = await api('/api/order', { method: 'POST', body: JSON.stringify({ target: 'table', target_id: selectedTableId, cart: payloadItems, source: 'staff', note: `staff-inline-${new Date().toISOString()}` }) });
  if (res.error) return;
  orderCart = [];
  await loadData();
  renderOrderCart();
...
async function loadData() {
  ...
  renderMenu();
  renderSystem();
  if (selectedTableId) renderExistingOrders(selectedTableId);
}
...
function bind() {
  ...
  qs('open-client-qr').addEventListener('click', () => { qs('client-qr-image').src = buildQrImageUrl(`${window.location.origin}/scan/staff`); qs('qr-modal').classList.remove('hidden'); });
  ...
  qs('admin-logout').addEventListener('click', () => { adminUnlocked = false; window.localStorage.removeItem('fakdu_admin_auth'); syncAdminUI(); showScreen('customer'); });
  ...
  qs('quick-down').addEventListener('click', () => { tableGridColumns = Math.max(2, tableGridColumns - 1); renderTables(); });
  qs('quick-mid').addEventListener('click', () => { tableGridColumns = 4; renderTables(); });
  qs('quick-up').addEventListener('click', () => { tableGridColumns = Math.min(6, tableGridColumns + 1); renderTables(); });

  qs('order-qty-minus').addEventListener('click', () => { qs('order-qty').value = Math.max(1, Number(qs('order-qty').value || 1) - 1); });
  qs('order-qty-plus').addEventListener('click', () => { qs('order-qty').value = Number(qs('order-qty').value || 1) + 1; });
  qs('order-add-item').addEventListener('click', () => {
    if (!selectedMenuItem) return;
    orderCart.push({ name: selectedMenuItem.name, price: Number(selectedMenuItem.price), qty: Math.max(1, Number(qs('order-qty').value || 1)), addon: qs('order-addon').value, note: qs('order-note').value.trim() });
    qs('order-note').value = ''; qs('order-qty').value = 1; renderOrderCart();
  });
  ...
  qs('open-add-menu').addEventListener('click', () => { menuEditIndex = -1; qs('menu-name').value = ''; qs('menu-price').value = ''; qs('menu-image').value = ''; qs('menu-addons').value = ''; });
  qs('save-menu').addEventListener('click', async () => {
    const name = qs('menu-name').value.trim(); const price = Number(qs('menu-price').value || 0);
    if (!name || price <= 0) return;
    const payload = { name, price, image: qs('menu-image').value.trim(), addons: qs('menu-addons').value.split(',').map((x) => x.trim()).filter(Boolean) };
    ...
    menuEditIndex = -1; qs('menu-name').value = ''; qs('menu-price').value = ''; qs('menu-image').value = ''; qs('menu-addons').value = '';
    await loadData();
  });
  ...
  document.querySelectorAll('.modal').forEach((modalEl) => modalEl.addEventListener('click', (event) => { if (event.target === modalEl) modalEl.classList.add('hidden'); }));

  qs('save-system').addEventListener('click', async () => {
    let qrImage = db.settings?.qrImage || '';
    const file = qs('qr-image').files?.[0];
    if (file) qrImage = await new Promise((resolve, reject) => { const r = new FileReader(); r.onload = () => resolve(r.result); r.onerror = reject; r.readAsDataURL(file); });
    await api('/api/settings', { method: 'POST', body: JSON.stringify({ settings: { storeName: qs('store-name').value.trim(), logoName: qs('store-logo-name').value.trim(), themeColor: qs('theme-color').value, bgColor: qs('bg-color').value, bankName: qs('bank-name').value.trim(), promptPay: qs('promptpay').value.trim(), adminPin: qs('admin-pin').value.trim() || 'admin', dynamicPromptPay: qs('dynamic-qr').checked, qrImage } }) });
    await loadData();
  });
  ...
  qs('backup-db').addEventListener('click', async () => {
    ...
    const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `fakdu-backup-${new Date().toISOString().slice(0, 10)}.json`; a.click(); URL.revokeObjectURL(url);
  });
